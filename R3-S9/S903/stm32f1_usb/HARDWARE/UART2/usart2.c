...
{		
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //接收到一个字节
	{
		...
		UART3_RX_BUF[UART3_RX_LEN++]=USART_ReceiveData(USART3);//(UART3->DR);	//读取接收到的数据
	}
	else if(USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)//接收到一帧数据（接收到空闲帧）
	{
		Res = USART3->DR;Res = Res;
		if((UART3_RX_LEN == 8) && ((u8)(UART3_RX_BUF[0]+UART3_RX_BUF[1]+UART3_RX_BUF[2]+
			UART3_RX_BUF[3]+UART3_RX_BUF[4]+UART3_RX_BUF[5]+UART3_RX_BUF[6]) == UART3_RX_BUF[7]))		//校验通过
		{
			Wind_Speed = ((UART3_RX_BUF[2]<<8)+ UART3_RX_BUF[3]);
		}
		UART3_RX_LEN = 0;
	}
...
static short Pm10 = 0;
//static short JiaQuan = 0;
//static short WenDu = 0;
//static short ShiDu = 0;
...
void USART2_IRQHandler(void)                	//串口3中断服务程序
{
	...
	else if(USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)//接收到一帧数据（接收到空闲帧）
	{
		...
		if((UART2_RX_LEN == 32) && (UART2_RX_BUF[0] == 0x42) && (UART2_RX_BUF[1] == 0x4d))		//校验通过
		{			
			Pm1_0 = ((UART2_RX_BUF[4]<<8)+ UART2_RX_BUF[5]);
			Pm2_5 = ((UART2_RX_BUF[6]<<8)+ UART2_RX_BUF[7]);
			Pm10 = ((UART2_RX_BUF[8]<<8)+ UART2_RX_BUF[9]);
		}
		
//		if((UART3_RX_BUF[0] == 0x42) && (UART3_RX_BUF[1] == 0x4d))		//校验通过
//		{				
//			Pm1_0 = ((UART3_RX_BUF[4]<<8)+ UART3_RX_BUF[5]);
//			Pm2_5 = ((UART3_RX_BUF[6]<<8)+ UART3_RX_BUF[7]);
//			Pm10 = ((UART3_RX_BUF[8]<<8)+ UART3_RX_BUF[9]);
//			JiaQuan = ((UART3_RX_BUF[28]<<8)+ UART3_RX_BUF[29]);
//			WenDu = ((UART3_RX_BUF[30]<<8)+ UART3_RX_BUF[31]);
//			ShiDu = ((UART3_RX_BUF[32]<<8)+ UART3_RX_BUF[33]);
//		}
