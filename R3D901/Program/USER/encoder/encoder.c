...
void ENC_Init2(void)//电机A码盘采集定时器，TIM3编码器模式
{
	...
	//左电机对应引脚复用映射
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource6,GPIO_AF_TIM3); //GPIOB4复用为TIM3
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource7,GPIO_AF_TIM3); //GPIOB5复用为TIM3
...
void ENC_Init(void)//电机处理初始化
{
    ENC_Init2();              //设置电机A TIM4编码器模式PB6 PB7 右电机
		ENC_Init1();              //设置电机D TIM3编码器模式PA12 PA13 左电机 
}
