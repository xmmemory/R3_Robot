...
//	uart5_init(9600);	//串口4初始化波特率为9600
	...
	MYDMA_Config(DMA1_Stream4,DMA_Channel_7,(u32)&USART3->DR,(u32)SendBuff2,SEND_BUF2_SIZE);//DMA2,STEAM7,CH4,外设为串口3,存储器为SendBuff,长度为:SEND_BUF_SIZE.
}
...
#define USART_REC_LEN  			200  								//定义最大接收字节数 100
...
void USART3_IRQHandler(void)                	
{
	...
	else if(USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)					//接收到一帧数据（接收到空闲帧）
	{
		...
		if(UART3_RX_LEN >= 10)
		{
			if((UART3_RX_BUF[0] == 0x05) && (UART3_RX_BUF[UART3_RX_LEN-1] == 0x07))							//校验通过
			{
				while(uart1_format_build_and_send(UART3_RX_BUF,0x01,0x01,0x01,UART3_RX_LEN));			//透传
