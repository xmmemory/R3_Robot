...
int main(void)
{
	...
	if(reset_pro)		//程序复位保护
	{		
		oriention = RTC_ReadBackupRegister(RTC_BKP_DR0)/1000.0;
		position_x = RTC_ReadBackupRegister(RTC_BKP_DR3)/1000.0;
		position_y = RTC_ReadBackupRegister(RTC_BKP_DR6)/1000.0;
		if(RTC_ReadBackupRegister(RTC_BKP_DR1) == 123)
			oriention = -oriention;
		if(RTC_ReadBackupRegister(RTC_BKP_DR4) == 123)
			position_x = -position_x;
		if(RTC_ReadBackupRegister(RTC_BKP_DR7) == 123)
	...
	/*系统正常运行的保护措施*/
	PVD_Config();							//低电压中断初始化
	IWDG_Init(IWDG_Prescaler_64,2000); //与分频数为64,重载值为2000,溢出时间为4s	
