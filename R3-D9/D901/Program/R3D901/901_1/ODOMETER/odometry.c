...
void odometry_cal(int left,int right)
{
  ...
  position_x = position_x + (float)(delta_distance * cos_ * const_frame);//计算出里程计x坐标
	...
	RTC_WriteBackupRegister(RTC_BKP_DR0,fabs(oriention)*1000);
	RTC_WriteBackupRegister(RTC_BKP_DR3,fabs(position_x)*1000);
	RTC_WriteBackupRegister(RTC_BKP_DR6,fabs(position_y)*1000);
	if(oriention < 0)
		RTC_WriteBackupRegister(RTC_BKP_DR1,123);
	if(position_x < 0)
		RTC_WriteBackupRegister(RTC_BKP_DR4,123);
	if(position_y < 0)
		RTC_WriteBackupRegister(RTC_BKP_DR7,123);
