...
{
	if(TIM_GetITStatus(TIM6,TIM_IT_Update)==SET)						//溢出中断	//500ms
	{
...
//	TIM3_Int_Init(999,839); 										//定时器时钟84M，分频系数840，所以84M/840=100Khz的计数频率，计数100次为1ms 
	...
	LEFT_DIR = 1;
	RIGHT_DIR = 1;
	TIM13_PWM_Init(5000-1,84-1);	//84M/84=1Mhz的计数频率,重装载值5000，所以PWM频率为 1M/5000=200hz.——PF8
	TIM14_PWM_Init(5000-1,84-1);	//84M/84=1Mhz的计数频率,重装载值5000，所以PWM频率为 1M/5000=200hz.——PF9
...
//	SensorToServer_Struct.d.pose_theta = 0;
	...
	SensorToServer_Struct.d.air_quality = air_quality.d*20;
	SensorToServer_Struct.d.voice1 = 6.34*pow(voice.d,3)-32.39*pow(voice.d,2)+59.65*pow(voice.d,1)+32.23;
	SensorToServer_Struct.d.voice2 = voice.d;
	SensorToServer_Struct.d.hydr = h2_value.d*100/12;
