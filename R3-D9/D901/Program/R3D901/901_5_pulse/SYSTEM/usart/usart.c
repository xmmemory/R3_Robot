...
sensor_rec temperature,humidity,air_velocity,air_quality,voice,h2_value,pm1_0,pm2_5,pm10;								//接收的传感器数据
...
*				       4) 累计和错误返回-4;
							 5) 设备id错误返回-4;
************************************************************************************************************************/
u8 A6_Format_Check(u8 *p_arg,u8 length)						//导航模块数据校验
{
	u16 check_sum= 0;
...
*				       4) 累计和错误返回-4;
							 5) 设备id错误返回-4;
************************************************************************************************************************/
u8 _05_Format_Check(u8 *p_arg,u8 length)					//服务器数据校验
{
	u16 check_sum= 0;
	for(int i=0;i<(length-5);i++)
	{
...
unsigned char UART1_RX_BUFF[128];     					//接收缓存区
...
void USART1_IRQHandler(void)                										//串口1中断服务程序
{
	...
	{
		Res = USART1->DR;
		if(!A6_Format_Check(UART1_RX_BUF,USART1_RX_STA) && !get_uart1_data && USART1_RX_STA)			//校验通过
		{
			get_uart1_data = 1;
			memcpy(UART1_RX_BUFF,UART1_RX_BUF+4,100);
...
#define UART2_REC_LEN  			200  									//定义最大接收字节数 200
...
void USART2_IRQHandler(void)                	//串口2中断服务程序.
{
	...
	{
		Res = USART2->DR;
		if((UART2_RX_BUF[0] == 0xA6) && (UART2_RX_BUF[1] == 0x59) &&(UART2_RX_BUF[2] == 0x2F) && (UART2_RX_BUF[46] == 0x54))//校验通过
		{
			memcpy(temperature.data,UART2_RX_BUF+6,0x2F);
			memset(UART2_RX_BUF,0,100);
...
* Note(s)    : 1) Null；
************************************************************************************************************************/
u8 uart3_format_build_and_send(u8 *arg,u8 device_id,u8 data_id,u8 length)		//数据格式整理及发送
{
	...
	if(DMA_GetFlagStatus(DMA1_Stream4,DMA_FLAG_TCIF4)!=RESET)//等待DMA1_Steam4传输完成
	{
		...
		//Time
		SendBuff3[0+4+2+2+1] = 20;
		SendBuff3[0+4+2+2+2] = 18;
		SendBuff3[0+4+2+2+3] = 12;
		SendBuff3[0+4+2+2+4] = 20;
		SendBuff3[0+4+2+2+5] = 15;
		SendBuff3[0+4+2+2+6] = 33;
		SendBuff3[0+4+2+2+7] = 56;
		//Data
		for(u8 j=0;j<length;j++)
		{
			SendBuff3[j+0+4+2+2+8]=*(arg+j);
		}
		//Check_Sum_Cal
		for(u8 check_count = 0;check_count < length+0+4+2+2+8;check_count++)		//“length+0+4+2+2+7”数据长度+起始15个字节
		check_sum += SendBuff3[check_count];		
		//Check_Sum
		SendBuff3[((length++)+0+4+2+2+8)] = (check_sum >> 8);
		SendBuff3[((length++)+0+4+2+2+8)] = check_sum;
		//Tail
		SendBuff3[((length++)+0+4+2+2+8)] = 0x07;//添加结束符		
		//DMA_Send
		DMA_ClearFlag(DMA1_Stream4,DMA_FLAG_TCIF4);		//清除DMA1_Stream4传输完成标志
		USART_DMACmd(USART3,USART_DMAReq_Tx,ENABLE);  //使能串口3的DMA发送
		MYDMA_Enable(DMA1_Stream4,length+0+4+2+2+8);  //开始一次DMA传输！
		//发送成功
		return 0;	
	}
	else
