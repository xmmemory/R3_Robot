...
static u8 quantity_of_electric;		//电池剩余电量比例
static u8 battert_check_flag;			//电量查询标志位
...
void system_init_step5(void)														//通讯初始化
{
	...
	while(!(USART3_RX_STA&0x8000))												//未接收到通讯模块通讯回应
	{
...
void odom_send_task(void)
{
	...
	while(uart1_format_build_and_send(odometry_data,DEVICE_ID_D9,COM_COMMOND,REG_ODOMETRY,20));	
}
...
//		system_init_step3();
	...
	while(1)
	{
		...
		if(Robot_Status == NAVI_STATUS)
		{
			if(battert_check_flag == 0)
			{
				quantity_of_electric = battery_check();
